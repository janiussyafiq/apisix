...
keywords:
  ...
  - LDAP Authentication
  - ldap-auth-advanced
description: The ldap-auth-advanced Plugin adds LDAP authentication with a search-then-bind flow, LDAP group retrieval, and group-based authorization to a Route or a Service.
---
...
|------|------|----------|-------------|
| user_dn | string | Conditional | Match the request when the resolved user DN equals this value. For example, `cn=user01,ou=users,dc=example,dc=org`. |
| group_dn | string | Conditional | Match the request when one of the user's group DNs equals this value. |

Exactly one of `user_dn` or `group_dn` must be set, and a `user_dn` match takes precedence over a
`group_dn` match. Both values are compared **verbatim** against the DNs resolved from the
directory; environment variable and secret manager references (`$env://`, `$secret://`) are not
resolved in these two fields.
...
| ldap_password | string | False | | | Password for `bind_dn`. Required when `bind_dn` is set. This field is encrypted with AES before being stored, and can also be referenced from an environment variable using the `env://` prefix, or a secret manager such as HashiCorp Vault using the `secret://` prefix. |
...
| header_type | string | False | ldap | ["ldap", "basic"] | Authentication scheme word the Plugin accepts in the credential header and writes in the `WWW-Authenticate` response header on a `401`. With the default `ldap`, credentials are sent as `Authorization: ldap <base64(user:password)>`; set to `basic` to accept the standard `Basic` scheme (for example, from `curl -u`). |
...
### Implement LDAP Authentication on the Route
...
Because Consumer association is required by default (`consumer_required`), first create a
Consumer whose `user_dn` matches the directory entry the login resolves to:
...
Create a Route protected by the Plugin:
...
The user then authenticates with the login name carried in the `uid` attribute. With the default
`header_type` of `ldap`, the credential is sent as `ldap <base64(user:password)>`:

```shell
curl -i "http://127.0.0.1:9080/hello" -H "Authorization: ldap $(echo -n 'jdoe:janesecret' | base64)"
```

You should receive an `HTTP/1.1 200 OK` response. A request with a wrong password receives an
...
To send credentials with the standard `Basic` scheme instead (for example, with `curl -u`), set
`header_type` to `basic` in the Plugin configuration on the Route.
...
A user who authenticates but is in none of the required groups receives an
`HTTP/1.1 403 Forbidden` response, distinct from the `401` returned for a failed authentication.
